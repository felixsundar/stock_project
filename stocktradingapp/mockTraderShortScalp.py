...
def placeEntryOrder(zerodha_user_id, kite, signal):
    ...
    if variety == CO_ORDER and 1 == 2: #place co order
        trigger_price = calculateCOtriggerPrice(token_co_upper_trigger[signal['instrument_token']], signal['current_price'])
...
def updateUserNetValue(user_id, position, exit_price, instrument_token):
    ...
    if exit_time_reached:
        loss_trades += 1
        loss_percent = (position['entry_price'] - exit_price) * 100.0 / position['entry_price']
