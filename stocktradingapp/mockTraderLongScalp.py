...
def checkStoploss(instrument_token, current_price, current_time):
    for position in current_positions[instrument_token]:
        if current_price >= position['target_price'] or exit_time_reached: # stoploss breached
...
def sendSignal(enter_or_exit, instrument_token, currentPrice_or_currentPosition): # 0 for exit, 1 for enter
    if enter_or_exit == ENTER:
        for signal_queue in signal_queues.values():
            try:
                signal = {'enter_or_exit': ENTER, 'instrument_token': instrument_token, 'current_price': currentPrice_or_currentPosition}
...
def placeEntryOrder(zerodha_user_id, kite, signal):
    ...
    if variety == CO_ORDER and 1 == 2: #place co order
        trigger_price = calculateCOtriggerPrice(token_co_lower_trigger[signal['instrument_token']], signal['current_price'])
        ...
        order_id = place_order(zerodha_user_id, variety, quantity, signal['current_price'], signal['instrument_token'])
    pending_orders[zerodha_user_id].append({'enter_or_exit':ENTER, 'order_id':order_id, 'instrument_token':signal['instrument_token']})
...
def updateUserNetValue(user_id, position, exit_price, instrument_token):
    ...
    if exit_time_reached:
        loss_trades += 1
        loss_percent = (exit_price - position['entry_price']) * 100.0 / position['entry_price']
...
def sendStatusEmail():
    ...
    try:
        ...
        monitor_status = 'Status for Mock Long Scalp at time : ' + str(now()) \
                         ...
                         + '\nTotal trades : ' + str(profit_trades + loss_trades) \
